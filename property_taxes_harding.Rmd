---
title: "Local Property Taxes - Template"
...
source("./Code/build_muni_data_files.R")
source("./Code/building_gis_data_files.R")
target.town <- "Harding Twp"
target.town.tax.data <- all.years.tax.data %>% filter(county.muni == target.town)
...
This version is a just a sample for discussion. More data verification is needed. 
...
These three files are downloaded from the Morris County (GIS) system, unzipped and converted from the ESRI shapefile format to a Simple Features format for processing. Here is an example of the parcels for `r target.town` together with an outline of Morris County. 
...
H.Parcels <- Parcels.sf %>% 
  filter(MuniID == 1413) 
  ggplot() + 
  geom_sf(data = H.Parcels, fill= "blue") +
  labs(title = paste("Example of Parcel Map: ", target.town)) + 
  theme_map()
...
AHO-1 Affordable Housing Overlay Zone 1

AHO-2 Affordable Housing Overlay Zone 2
...
write.csv(data.out, file = "./Data_Out/harding_taxes_year.csv")
...
H.Parcels %>% mutate(residential= ifelse(PropCode == "2", T, F)) %>% 
    ggplot() + 
...
H.Parcels %>% mutate(qfarm = ifelse(PropCode == "3B", T, F)) %>% 
    ggplot() + 
...
H.Parcels %>% mutate(qfarm = ifelse(PropCode == "3A", T, F)) %>% 
    ggplot() + 
  geom_sf(aes(fill = as.factor(qfarm))) + 
...
target.town.tax.data %>% 
  group_by(ZONING) %>%  
  summarize(acres = sum(round(as.numeric(CALCULATED_ACREAGE)/10000,0))) %>%  
...
Seems to be close to last sale date in zillow but zillow doesn't seem to have everything and vice versa
